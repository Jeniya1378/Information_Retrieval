...
# this function reads each file in the directore
...
def process_text_file(file_path, custom_stopwords):
    text = read_text(file_path)
    tokens = tokenize_and_stem(text)
    filtered_tokens = remove_stopwords(tokens, custom_stopwords)
    term_frequency = calculate_term_frequency(filtered_tokens)
    return term_frequency

# this function combines term frequency calculation and document id extraction process together
def process_file(file_path, custom_stopwords):
    doc_id = extract_doc_id(file_path)
    term_frequency = process_text_file(file_path, custom_stopwords)
...
def process_text_dataset(file_paths, custom_stopwords):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for file_path in file_paths:
            future = executor.submit(process_file, file_path, custom_stopwords)
            try:
...
def main():
    ...
    term_frequency_dataset = process_text_dataset(file_paths, custom_stopwords)
